...
import { ErrorMessage, Select, Button, Modal, FormatNumber } from "@/common";
...
export default function Index({ info }) {
    ...
    const getInvoice = (data) => {
        if (userSign && assistanceSign) {
            ...
            useAxios.post(api.rescuerInvoice.getTechnicianAndInsuranceReg, params).then(res => {
    ...
    return <section className="w-[1200px] max-w-full m-auto">
        <section className="grid grid-cols-1 xl:grid-cols-2 gap-5">
            <section className="grid grid-cols-1 gap-y-4 xl:grid-cols-2 relative border border-[#f7f7f7] bg-white shadow-xl p-5 pr-[80px] rounded-md">
                ...
                <LabelValue label="سقف تعهد:" value={`${numberWithCommas(userInfo?.remainingCoverageAmount)} تومان`} />
            </section>
                    ...
                    <Controller
                        ...
                        render={({ field: { onChange, value } }) => (
                            <FormatNumber
                                setState={onChange}
                                state={value}
                                placeholder="ارزش خودروی امدادخواه را وارد کنید"
                                className="h-[48px] rounded-lg "
                            />
                        )}
                    />
